// M13 NODE — USO MTU SLICER & EDT HARDWARE PACER
//
...
/// Convenience function: compute fragment count and return iterator.
/// Wires `total_slices()` into production fragmentation paths (send_fragmented_udp)
/// for pre-flight validation + debug telemetry of handshake fragment geometry.
#[inline]
pub fn slice_uso_counted(payload: &[u8], mtu: usize) -> (UsoSliceIter<'_>, usize) {
    let iter = slice_uso(payload, mtu);
    let total = iter.total_slices();
    (iter, total)
}
