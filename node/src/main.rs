...
fn run_udp_worker(hub_addr: &str, echo: bool, hexdump_mode: bool, mut tun: Option<std::fs::File>) {
    ...
    // [P1-02] STRICT USO MTU SIZING: tx_bufs pre-allocated strictly to MAX_FRAME_SIZE (1442).
    const TUN_BATCH: usize = 64;
    const MAX_FRAME_SIZE: usize = ETH_HDR_SIZE + M13_HDR_SIZE + USO_MTU;
    let mut tx_bufs: [[u8; MAX_FRAME_SIZE]; TUN_BATCH] = [[0u8; MAX_FRAME_SIZE]; TUN_BATCH];
    ...
    loop {
        ...
        if now.saturating_sub(last_report_ns) > 1_000_000_000 {
            ...
            eprintln!("[M13-N0] RX:{} TX:{} TUN_R:{} TUN_W:{} AEAD_OK:{} FAIL:{} State:{} Up:{}s DL:{}kbps UL:{}kbps",
        ...
        if let Some(ref mut tun_file) = tun {
            ...
            if let NodeState::Established { ref cipher, .. } = state {
                ...
                for _ in 0..TUN_BATCH {
                    ...
                    // Mathematically ensures L2 frame never exceeds 1442 bytes (62 + 1380).
                    // Eliminates geometric loss over RF caused by kernel IP fragmentation.
                    match tun_file.read(&mut frame[62..62 + USO_MTU]) {
                        Ok(n) if n > 0 => {
                ...
                if tx_count_batch > 0 {
                    ...
                    if sent > 0 {
                        ...
                        for si in 0..sent as usize { tx_bytes += tx_lens[si] as u64; }
                    }
                }
            }
        }
